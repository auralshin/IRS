...
contract IRSHookTest is Test {
    ...
    function setUp() public {
        ...
        // 2. Deploy local base index + margin manager
        base = new EthBaseIndex(address(this), 95e9);
        margin = new MarginManager(address(this));

        // 3. Deploy factory that builds IRS pools
        factory = new IRSPoolFactory(manager);

        // 4. Whitelist alice, not bob
    ...
    function test_HookAddressHasCorrectFlags() public {
        ...
        uint160 flags = uint160(
            uint160(uint160(uint160(uint160(uint160(uint160(hookAddr))))))
        );
    ...
    function test_MarginGate_AllowsWhitelisted() public {
        ...
        (bytes4 sel, BeforeSwapDelta d, uint24 optFee) = IRSHook(hookAddr)
            .beforeSwap(address(this), key, sp, "");
        vm.stopPrank();
