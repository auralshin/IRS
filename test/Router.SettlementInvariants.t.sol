...
contract Router_SettlementInvariants is Test {
    ...
    uint256 r1Before = token1.balanceOf(user);
        ...
        IRSV4Router.SwapEx memory sx = IRSV4Router.SwapEx({
            key: key,
            params: SwapParams({ zeroForOne: true, amountSpecified: int256(1), sqrtPriceLimitX96: uint160(0) }),
            hookData: bytes("") ,
            maxPay: 250 ether,
            useNative: false
        });
        ...
        assertEq(token0.balanceOf(address(manager)) - m0Before, 250 ether, "manager must get T0 pay");
    ...
    assertEq(int256(token1.balanceOf(address(manager))) - int256(m1Before), -int256(180 ether), "manager T1 out");
